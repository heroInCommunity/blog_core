...
<link rel="stylesheet" href="<%=request.getAttribute(AttributeName.BASE_URL.value()) %>css/selectize.bootstrap3.css">
					...
					<div class="col-xs-12 col-sm-9 well admin-fields-area">
						<div role="form">
							...
							<div class="list-group search-words-group well">
								<input type="text" id="tag_word" value="
								<%
								int count = 0;
								for(Tag tag : ( (Post) request.getAttribute(AttributeName.POST.value()) ).getTags()) {
									if(count != 0) {
										out.print(",");
									}
							...
							<button type="submit" id="edit_post" class="btn btn-default">Update</button>
						</div>
					</div>
				</div>
			</div>
		...
		$( document ).ready(function() {
	        ...
	        // create editor
	        $('#postBody').elrte(opts);
	        $('#postBody').elrte('val', '<%=( (Post) request.getAttribute(AttributeName.POST.value()) ).getBody()%>');
